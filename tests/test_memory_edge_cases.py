...
class TestSearchEdgeCases:
    ...
    def test_search_all_filters_intersection(self, store, conn):
        store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="semantic",
                            access_scope="workgroup", content={"target": "yes"},
                            importance=0.8)
        store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                            content={"target": "no"}, importance=0.8)
        store.create_memory(agent_id="a2", workgroup_id=WG, memory_type="semantic",
                            content={"target": "no"}, importance=0.8)
        r = search_memories(conn, WG, agent_id="a1", memory_type="semantic",
                            min_importance=0.7, content_contains="yes",
                            caller_agent_id="a1", limit=100)
        assert len(r) == 1
