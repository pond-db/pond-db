...
class TestJwtEnforcement:
    ...
    def test_different_workgroup_creates_in_that_wg(self, store):
        m = store.create_memory(agent_id="a1", workgroup_id="other-wg",
    ...
    def test_feedback_rate_limit_10_per_hour(self, store, conn):
        ...
        for i in range(10):
            write_access_log(conn, agent_id="a1", workgroup_id=WG, action="feedback",
                             memory_ids=[m["id"]])
        count = count_recent_actions(conn, action="feedback", memory_id=m["id"],
                                     window_seconds=3600)
        assert count == 10
...
class TestCausalChain:
    def test_direct_cycle_detected(self, store):
        m1 = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                                 content={"step": 1})
        m2 = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                                 content={"step": 2}, causal_parent_id=m1["id"])
    ...
    def test_three_node_cycle_detected(self, store):
        ...
        m2 = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                                 content={"step": 2}, causal_parent_id=m1["id"])
        m3 = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                                 content={"step": 3}, causal_parent_id=m2["id"])
    ...
    def test_no_cycle_linear_chain(self, store):
        ...
        for i in range(5):
            ...
            m = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                                    content={"step": i}, causal_parent_id=parent)
            mems.append(m)
    ...
    def test_deep_chain_up_to_50(self, store):
        ...
        for i in range(50):
            parent = mems[-1]["id"] if mems else None
            m = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="episodic",
                                    content={"depth": i}, causal_parent_id=parent)
            mems.append(m)
...
class TestWorkingMemoryExpiry:
    def test_expired_working_memory_not_in_search(self, store, conn):
        store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="working",
                            access_scope="private", content={"tmp": True},
    ...
    def test_non_expired_working_memory_in_search(self, store, conn):
        store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="working",
                            access_scope="private", content={"active": True},
...
class TestSoftDelete:
    ...
    def test_deleted_has_deleted_at_in_db(self, store, conn):
        m = store.create_memory(agent_id="a1", workgroup_id=WG, memory_type="semantic",
                                content={"x": 1})
        store.soft_delete_memory(m["id"])
        row = conn.execute("SELECT deleted_at FROM agent_memories WHERE id = ?",
                           (m["id"],)).fetchone()
...
class TestUtilityDecay:
    def test_decay_reduces_old_memories(self, store):
        ...
        store._conn.execute("UPDATE agent_memories SET last_accessed_at = ? WHERE id = ?",
                            (old, m["id"]))
        store._conn.commit()
